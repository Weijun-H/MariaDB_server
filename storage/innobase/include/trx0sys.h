...
class rw_trx_hash_t
{
  ...
  hash_chain &cell_get(trx_id_t id) const { return array[calc_hash(id)]; }
...
public:
  bool is_initialised() const { return array!= nullptr; }

  /** Create the hash table.
  @param n  the lower bound of n_cells */
  inline void create(ulint n);

  void destroy() { aligned_free(array); n_cells= 0; array= nullptr; }
  ...
  page_hash_latch &lock_get(trx_id_t id) const
  { return lock_get(cell_get(id)); }
...
public:
  ...
  */
  ...
  bool is_initialised() const { return rw_trx_hash.is_initialised(); }
